...
- [x] `NeuroRuneUITests` target 재생성 (기존 targets에서 삭제된 상태)
- [x] `LaunchArguments` / `LaunchEnvironment` 기반 Mock 주입 구조
...
- [x] 각 화면에 `accessibilityIdentifier` 정리
  - 채팅 입력 필드, 전송 버튼, 메시지 버블(role 태깅), mic 버튼, 쓰기 승인 모달 버튼
- [x] UI 테스트 시드 데이터 리셋 (SwiftData in-memory 옵션 or 앱 기동 시 clean)
- [x] Continuation gate 패턴 활용 — wall-clock 대기 금지

### Smoke 1. 채팅 기본 플로우

- [x] **목적**: 앱이 기동되고, 사용자가 메시지를 전송하면 assistant 응답 버블이 나타난다
- [x] **스텝**:
...
- [x] **Mock**: Anthropic 스트리밍 응답을 고정 delta 2~3개 + stop
- [x] **주의**: 스트림 지연은 ImmediateClock + Continuation gate로 결정적으로

### Smoke 2. STT → autoSend → assistant 응답

- [x] **목적**: 음성 입력이 transcribe되면 countdown 만료 시 자동 전송되고 assistant 응답이 온다
- [x] **스텝**:
...
- [x] **Mock**: `STTClient.transcribe`가 고정 문자열 반환
- [x] **주의**: 시뮬레이터에서 실제 mic 동작 불가 → STTClient stub이 핵심. countdown은 ImmediateClock/TestClock으로 advance

### Smoke 3. memory write 승인 모달

- [x] **목적**: assistant가 `write_memory` tool을 호출하면 승인 모달이 뜨고, accept 시 파일이 기록된다
- [x] **스텝**:
...
- [x] **Mock**: Anthropic 응답이 tool_use(`write_memory`, filename, content) 포함. GitHubClient.saveFile도 stub
- [x] **주의**: `WriteApprovalGate`는 실제 구현 사용 (승인 플로우 검증이 목적). diff 렌더링 LineDiff는 실제 로직 돌림

### 측정 목표

- [ ] UI smoke 3건 포함 전체 테스트 사이클 **40초 이내** 유지 — 콜드 실행 실측 약 63초. 목표 미달, 기준을 올릴지 UI 부팅 비용을 줄일지 판단 필요
- [ ] CI 실행 시간 증가 30초 이내 — 미측정
- [ ] flaky 0건 (10회 연속 돌려 확인) — 미측정
